...
public class UnitFacadeREST extends AbstractFacade<UnitEntity> {
  ...
  private UnitPKEmbeddable getPrimaryKey(PathSegment pathSegment) {
     ...
     * is supposed to be in form of 'somePath;magni=magniValue;unitId=unitIdValue'. Here 'somePath'
     * is a result of getPath() method invocation and it is ignored in the following code. Matrix
     * parameters are used as field names to build a primary key instance.
     */
    mx.ipn.escom.supernaut.nile.model.UnitPKEmbeddable key =
        new mx.ipn.escom.supernaut.nile.model.UnitPKEmbeddable();
  ...
  @Produces({MediaType.APPLICATION_XML + "; charset=UTF-8",
      MediaType.APPLICATION_JSON + "; charset=UTF-8"})
  public List<UnitEntity> findRange(@PathParam("from") Integer from,
