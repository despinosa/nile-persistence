...
public class AddressFacadeREST extends AbstractFacade<AddressEntity> {
  ...
  private AddressPKEmbeddable getPrimaryKey(PathSegment pathSegment) {
     ...
     * is supposed to be in form of 'somePath;customer=customerValue;type=typeValue'. Here
     ...
     */
    mx.ipn.escom.supernaut.nile.model.AddressPKEmbeddable key =
        new mx.ipn.escom.supernaut.nile.model.AddressPKEmbeddable();
  ...
  @Produces({MediaType.APPLICATION_XML + "; charset=UTF-8",
      MediaType.APPLICATION_JSON + "; charset=UTF-8"})
  public List<AddressEntity> findRange(@PathParam("from") Integer from,
